...
func main() {
	...
	localAddr := flag.String("local-addr", fmt.Sprintf("%s:5060", localIP), "SIP Local listening address")
	serverAddr := flag.String("server-addr", "192.168.18.13:5060", "SIP Server address")
	ice := flag.String("ice-server", "stun:192.168.18.13:3478", "ICE URL")
	httpAddress := flag.String("http-addr", ":8082", "HTTP Service Address")
	mediaPort := flag.Int("mediaPort", 50000, "Media Ports")
	userName := flag.String("userName", "snail", "username")
	displayName := flag.String("displayName", "snail", "Display Name")
...
func call(w http.ResponseWriter, r *http.Request) {
	...
	err := decode(sd, offer)
	if err != nil {
		resp(w, 400, "SDP mistake", nil)
		return
	}
	
	if userID == "" {
		resp(w, 400, "The other party's username is incorrect", nil)
		return
	}
